...
export default function AddToListsModal({ tmdb_id, onClose }) {
  ...
  const handleAdd = () => {
    selectedLists.forEach((lst) => addItems({ tmdb_id, listId: lst.id }));
    setSelectedLists([]);
    onClose();
  };
  ...
  return (
    <div className='fixed inset-0 z-999 flex items-center justify-center p-4'>
      ...
      <div
        ...
        className='bg-zinc-900 border z-2 border-zinc-700 rounded-sm p-4 flex flex-col gap-3 w-full max-w-md text-zinc-200 shadow-xl transition-all duration-300 ease-out'
      >
        <h2 className='font-semibold tracking-widest'>
          Add {tmdb_id} to lists
        </h2>
        ...
        <div className='flex flex-col gap-2 max-h-48 overflow-y-auto'>
          {listsToShow.map((lst) => {
            const exists = lst.items?.some(
              (i) => Number(i.tmdb_id) === Number(tmdb_id)
            );
