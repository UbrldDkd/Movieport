    ...
    const fetchContent = async () => {
      try {
        ...
        const tmdbRes = await fetch(
          `${Url1}${mediaType}/${id}?api_key=${API_KEY1}&language=en-US`
        );
        ...
        if (tmdbData.imdb_id) {
          const omdbRes = await fetch(
            `${Url2}${tmdbData[details1.imdb_id]}&apikey=${API_KEY2}&plot=full`
          );
        ...
        setContent({
          ...
          title:
            mediaType === 'movie'
              ? tmdbData[details1.title]
              : tmdbData[details1.titleTv],
        });
