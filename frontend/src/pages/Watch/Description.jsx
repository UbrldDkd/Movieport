  ...
  const getGenreIdFromName = (genreName) => {
    ...
    for (const word of words) {
      const partialMatch = Object.entries(GenreMap).find(
        ([id, name]) => name.toLowerCase() === word.toLowerCase().trim()
      );
  ...
  const ratings = omdb?.[details2.ratings]?.length
    ...
    : tmdb
      ? [
          {
            Source: 'TMDb',
            Value: `${tmdb.vote_average} / 10 (${tmdb.vote_count} votes)`,
          },
        ]
      : [];
  ...
  const displayPosterUrl = !isLoading
    ? mediaType === 'movie' || selectedSeason === null
      ? tmdb?.[details1.poster]
        ? `https://image.tmdb.org/t/p/w500/${tmdb[details1.poster]}`
        : omdb?.[details2.poster] && omdb[details2.poster] !== 'N/A'
          ? omdb[details2.poster]
          : null
      : seasonContent?.[details1.poster]
        ? `https://image.tmdb.org/t/p/w500/${seasonContent[details1.poster]}`
        : tmdb?.[details1.poster]
          ? `https://image.tmdb.org/t/p/w500/${tmdb[details1.poster]}`
          : null
    : null;
    ...
    <div className='flex flex-col md:flex-row md:space-x-8 py-4'>
      ...
      <div className='text-sm space-y-2 flex flex-col'>
        ...
        {isLoading ? (
          ...
          <div
            style={{ fontSize: '37px' }}
            className='font-semibold text-zinc-300 transition-opacity duration-500 opacity-100'
          >
            {title}
          </div>
        )}
          ...
          <div className='flex text-xs space-x-3 font-normal mb-7'>
            {ratings.map((r) => {
              ...
              return (
                <p
                  key={r.Source}
                  className='flex items-center font-semibold text-xs space-x-2'
                >
                  <span>
            ...
            <div
              className={`h-full w-0.5 bg-gradient-to-t from-zinc-950 via-red-950 to-zinc-950${runtime ? 'opacity-100' : 'opacity-0'}`}
            />
            <p>{runtime}</p>
          </div>
        )}
                ...
                <>
                  ...
                  <div className='text-sm text-zinc-400 flex-1'>
                    {countries.map((country, index) => (
                      <span key={index}>
                        <Link
                          to={`/${mediaType}?countries=${countries}&match=any`}
                      ...
                      <div className='space-y-1'>
                        {castWithRoles.map((actor, index) => (
                          <div key={index} className='text-sm text-zinc-400'>
                            <span className='text-zinc-300'>{actor.name}</span>{' '}
                            as{' '}
                  ...
                  <div className='flex-1'>
                    ...
                    >
                      <span>
                        {crewExpanded
                          ? 'Hide crew'
                          : `Show crew (${Object.keys(crewByDepartment).length} departments)`}
                      </span>
                      <span
                        className={`transform transition-transform duration-200 ${crewExpanded ? 'rotate-180' : ''}`}
                      >
                        ↓
                      </span>
                    </button>
