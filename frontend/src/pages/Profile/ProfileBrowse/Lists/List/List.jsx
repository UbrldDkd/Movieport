...
export default function List() {
  ...
  const { list: fetchedList, loading, error } = useGetList(username, title_slug);
  ...
  const watchedCount = useMemo(() => {
    ...
    return user.contentRelations.filter((cr) => ids.has(cr.tmdb_id) && cr.watched).length;
  }, [list, user]);
  ...
  if (!list || error) {
    return (
      <div className="min-h-screen w-full bg-zinc-950 text-zinc-200 flex items-center justify-center">
        <div className="text-zinc-400">List not found</div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen w-full bg-zinc-950 text-zinc-200 overflow-x-hidden">
      <div className="mx-auto max-w-[1000px] mt-2 grid grid-cols-1 lg:grid-cols-[6fr_1.3fr] gap-2.5 min-w-0">
        {/* Main content */}
        <div className="bg-zinc-900/90 p-3 border border-zinc-800 rounded-sm overflow-hidden">
          <ListHeader list={list} username={username} view={view} setView={setView} />
          ...
          {totalPages > 1 && (
            <div className="flex justify-between items-center mt-6 text-xs text-zinc-400 px-2 py-1">
              <button onClick={prevPage} disabled={page === 1} className="disabled:opacity-50 disabled:cursor-not-allowed hover:text-zinc-200 transition-colors">
                Previous
              </button>
              <span className="text-zinc-300">Page {page} of {totalPages}</span>
              <button onClick={nextPage} disabled={page === totalPages} className="disabled:opacity-50 disabled:cursor-not-allowed hover:text-zinc-200 transition-colors">
                Next
              </button>
            </div>
          )}
        ...
        <div className="flex flex-col gap-2">
          ...
          <WatchedPercentage watchedCount={watchedCount} totalCount={list.items.length} />
        </div>
      </div>
    </div>
  );
