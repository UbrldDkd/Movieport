...
export function useListHandlers({
  ...
  const handleAddItem = (tmdb) => {
    const tmdb_id = tmdb.id;

    setNewList((prev) => {
      // Skip if item already exists
      if (prev.items.some((i) => i.tmdb_id === tmdb_id)) return prev;
    ...
    // Remove from itemsToRemove if present
    if (mode === 'edit') {
      setItemsToRemove((prev) => prev.filter((i) => i !== tmdb_id));
    }
  };
