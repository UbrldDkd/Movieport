...
export default function ContentCardActions({
  ...
  const tmdb_id = contentRelation?.tmdb_id;
  ...
  return (
    <div
      className={`absolute top-[0.5px] left-1/2 transform -translate-x-1/2
                 ...
                 ${view == 'lg' ? 'w-[65%] h-[15%]' : view == 'md' ?'w-[85%] h-[25%]' :'w-[85%] h-[23%]' }
                 `}
    >
      {/* Watched */}
      <button
        onClick={() => toggleField(tmdb_id, 'watched')}
        className='relative w-6 h-6 flex items-center justify-center pointer-events-auto hover:cursor-pointer'
      >
        <AiFillEye
          className={`${contentRelation?.watched ? 'opacity-100' : 'opacity-0'} absolute text-amber-200/75 ${view == 'lg' ? 'w-5 h-5' : view == 'sm' ? 'w-4 h-4' : ''}`}
        />
        <AiOutlineEye
          className={`${!contentRelation?.watched ? 'opacity-100' : 'opacity-0'} absolute text-gray-400 ${view == 'lg' ? 'w-5 h-5' : 'w-4 h-4'}`}
        />
      </button>

      {/* Likes */}
      <button
        onClick={() => toggleField(tmdb_id, 'likes')}
        className='relative w-6 h-6 flex items-center justify-center pointer-events-auto hover:cursor-pointer'
      >
        <VscHeartFilled
          className={`${contentRelation?.liked ? 'opacity-100' : 'opacity-0'} absolute text-red-800 ${view == 'lg' ? 'w-5 h-5' : view == 'sm' ? 'w-4 h-4' : ''}`}
        />
        <VscHeart
          className={`${contentRelation?.liked ? 'opacity-0' : 'opacity-100'} absolute text-gray-400 ${view == 'lg' ? 'w-5 h-5' : view == 'sm' ? 'w-4 h-4' : ''}`}
        />
      </button>
