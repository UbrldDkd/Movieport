  ...
  useEffect(() => {
    const fetchContent = async () => {
      try {
        ...
        const yearQuery = selectedYears.length > 0
        ...
        const languageQuery = selectedCountries.length === 0 ? `&with_original_language=en` : '';
        ...
        if (mediaType) {
          if (mediaType === 'movie') {
            ...
            sortQuery = noExtraFilters
              ? `&sort_by=${details.releaseDateTv}.desc,popularity.desc&${details.releaseDateTv}.lte=${today}`
              : `&sort_by=${details.releaseDateTv}.desc&${details.releaseDateTv}.lte=${today}`;
          }
        }

        const countryList = selectedCountries.length > 0 ? selectedCountries : [null];
        ...
        const allContent = resData
          ...
          .filter(
            item =>
              (item[details.poster] && item[details.title]) ||
              (item[details.titleTv] && item[details.id])
          )
          .sort((a, b) => {
            ...
            const dateA = new Date(a[details.releaseDate] || a[details.releaseDateTv] || 0);
            const dateB = new Date(b[details.releaseDate] || b[details.releaseDateTv] || 0);
            return dateB - dateA;
          })
