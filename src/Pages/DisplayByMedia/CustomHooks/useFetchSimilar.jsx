...
export function useFetchSimilar({ id, mediaType, currentPage, contentPerPage }) {
    ...
    const fetchSimilar = async () => {
      try {
        ...
        const fetchPromises = Array.from({ length: 4 }, (_, i) => 
          fetch(`${Url}${mediaType}/${id}/similar?api_key=${API_KEY}&page=${startPage + i}`)
        );
        ...
        if (currentPage === 1 && resData[0]?.total_results) {
          const totalUserPages = Math.ceil(resData[0].total_results / contentPerPage);
