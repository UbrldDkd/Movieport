...
export const DeadEndFilters = ({ genreIds, selectedYears, selectedCountries, value }) => {
  return (
    <div className="flex flex-col items-center justify-center min-h-screen text-center text-zinc-400 px-4 relative">
      ...
      <div className="flex flex-wrap items-center justify-center gap-2 text-base max-w-[90%] relative z-10">
        ...
        {genreIds.length > 0 && (
          <span className="break-words">
            <span className="text-zinc-200"> Genres:</span> {genreIds.map((id) => GenreMap[id]).join(', ')}
          </span>
        )}
        ...
        {selectedYears.length > 0 && (
          <span className="break-words">
            <span className="text-zinc-200"> Years:</span> {selectedYears.join(', ')}
          </span>
        )}
        ...
        {selectedCountries.length > 0 && (
          <span className="break-words">
            <span className="text-zinc-200"> Countries:</span> {selectedCountries.join(', ')}
          </span>
        )}
      </div>
    </div>
