...
export default function Filterbox() {
 ...
 function toggleDropdown() {
  setIsOpen(prev => !prev);
  if (!isOpen) { // only reset selections when opening
    setSelectedGenres([]);
    setSelectedYears([]);
...
useEffect(() => {
  ...
  ref={dropdownRef}
  className={`
    absolute top-full bg-zinc-900 rounded-md drop-shadow-black px-2 py-1.5 z-10 grid grid-cols-3 grid-rows-3 gap-2 max-w-[430px]
...
{/* Left Panel for filtering by years*/}
   <div className={`
  absolute top-0 left-[-220px] h-full w-52 bg-zinc-900 text-white rounded-md z-10 p-2 flex flex-col
  transition-all duration-300
  ${leftPanelOpen 
      ? 'translate-x-0 opacity-100'      
      : 'translate-x-full opacity-0 pointer-events-none'}      // exit: slide right and fade out
`}>

    <button
      onClick={() => setLeftPanelOpen(false)}
      className="absolute top-2 left-2 h-6 w-6 bg-red-950 text-white rounded-full flex items-center justify-center hover:bg-red-900 z-20 cursor-pointer"
    >
      x
    </button>
...
{/* Right Panel for filtering by countries */}
  ...
  ${rightPanelOpen 
      ? 'translate-x-0 opacity-100' 
      : '-translate-x-full opacity-0 pointer-events-none'}     // exit: slide left and fade out
`}>
    ...
    {/* Gradient overlays */}
    <div className="absolute top-0 left-0 h-full w-0.5 bg-gradient-to-b from-zinc-900 via-red-950 to-zinc-900 rounded-r-md pointer-events-none"></div>
    <div className="pointer-events-none absolute top-0 left-0 h-full w-full bg-gradient-to-b from-zinc-900 via-transparent to-zinc-900 rounded-md"></div>
  </div>
        ...
        {genres.map(([id, name ]) => (
          ...
          />
            ...
            className={`flex-1 text-sm px-1 py-2 rounded transition-colors duration-150 flex items-center justify-center gap-1
            ${mediaType === 'movie' ? 'bg-red-950 text-white' : 'bg-zinc-500 text-zinc-950 hover:text-zinc-300 cursor-pointer'}`}
            >
            ...
            className={`flex-1 text-sm px-1 py-1 rounded transition-colors duration-150 flex items-center justify-center gap-1
